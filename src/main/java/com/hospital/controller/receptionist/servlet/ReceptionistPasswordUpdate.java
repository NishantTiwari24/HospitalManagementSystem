...
/**
 * Servlet implementation class ReceptionistPasswordUpdate
 */
@WebServlet("/Receptionist/PasswordUpdate")
public class ReceptionistPasswordUpdate extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ReceptionistPasswordUpdate() {
        super();
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            if (!newPassword.equals(confirmPassword)) {
                ...
                request.getRequestDispatcher("/Receptionist/Profile").forward(request, response);
                return;
            }

            UserDao receptionistDao = new UserDao(con);

            boolean success = receptionistDao.isPasswordCorrectForUser(currentPassword,userId);
            if (!success) {
            ...
            boolean updatePasswordSuccess = receptionistDao.userUpdatePassword(userId, hashedNewPassword);
